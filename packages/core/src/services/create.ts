...
// Appends to existing queue instead of clearing it
export async function createAppend(params: CreateParams) {
  const { signer, addresses, amount, mintAddress, db } = params
  ...
  // This function APPENDS to the existing queue instead of clearing it
  ...
  logger.info(`Appended airdrop queue for ${addresses.length} addresses`)
}
